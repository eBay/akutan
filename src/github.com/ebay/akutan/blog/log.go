...
func (err TruncatedError) Error() string {
	return fmt.Sprintf("Akutan log truncated before requested index (%v)",
		err.Requested)
}
...
// A ClosedError is returned when attempting to use a AkutanLog after the log's
// background context has been canceled. This error indicates the AkutanLog
// instance is no longer useful and the process is likely shutting down.
type ClosedError struct{}

// Error implements the method defined by 'error'.
func (err ClosedError) Error() string {
	return "AkutanLog client has been closed (client shutdown)"
}
...
// A Factory creates a AkutanLog instance. The only errors returned are fatal
...
// New returns a new AkutanLog instance. It assumes the factory corresponding to
// cfg.AkutanLog (default "kafka") has been registered in the Factories map. It
...
func New(ctx context.Context, cfg *config.Akutan) (AkutanLog, error) {
	factory, ok := Factories[cfg.AkutanLog.Type]
	if !ok {
		return nil, fmt.Errorf("no AkutanLog %v factory found", cfg.AkutanLog.Type)
	}
	locator, err := discoveryfactory.NewLocator(ctx, &cfg.AkutanLog.Locator)
	if err != nil {
		return nil, err
	}
