...
func Test_NewLocator(t *testing.T) {
	...
	defer cancel()
	loc, err := kube.NewLocator(ctx, "grpc", "akutan/type in (diskview)")
	assert.NoError(err)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			locator := &kubeLocator{
				...
				selector: "akutan/type in (diskview)",
				current:  discovery.NewStaticLocator(nil),
			}
			// Setup
...
func Test_KubeLocator_String(t *testing.T) {
	...
	assert.Equal("Empty KubeLocator(portName=grpc, selector={akutan/type in (diskview)})",
		locator.String())

	locator.current.Set([]*discovery.Endpoint{
		&discovery.Endpoint{Network: "tcp", Host: "diskview-1", Port: "9980"},
	})
	assert.Equal("KubeLocator(portName=grpc, selector={akutan/type in (diskview)}, tcp://diskview-1:9980)",
		locator.String())
	...
	assert.Equal("KubeLocator(portName=grpc, selector={akutan/type in (diskview)}, len=2, first=tcp://diskview-1:9980)",
		locator.String())
}
...
func stringify(result discovery.Result) string {
	...
	for _, e := range result.Endpoints {
		...
		if val, exists := e.Annotations["akutan/partition"]; exists {
			b.WriteString(fmt.Sprintf("|p=%v", val))
		}
		if val, exists := e.Annotations["akutan/partitionCount"]; exists {
			b.WriteString(fmt.Sprintf("|pc=%v", val))
		}
		if val, exists := e.Annotations["akutan/features"]; exists {
...
func (spec mockPod) build() *corev1.Pod {
	...
	set("akutan/partition", spec.partition)
	set("akutan/partitionCount", spec.numPartitions)
	if len(spec.features) > 0 {
		a["akutan/features"] = strings.Join(spec.features, ",")
	}
