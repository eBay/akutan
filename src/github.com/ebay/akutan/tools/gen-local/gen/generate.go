...
// A Spec specifies the parameters of the cluster. It does not describe the Akutan
// log or other dependencies; it only describes the Akutan servers.
type Spec struct {
...
// A Cluster is the output of Spec.Generate. It has one entry for every Akutan
// server, in some reasonable order. It does not describe the Akutan log or other
// dependencies; it only describes the Akutan servers.
...
// "akutan-diskview" in the hashPO space.
...
// "akutan-diskview" in the hashSP space.
...
func (spec *Spec) diskViewCfg(viewCounter *int) *config.Akutan {
	cfg := copyConfig(spec.BaseCfg)
	cfg.API = nil
	cfg.TxTimeoutView = nil
	if cfg.DiskView == nil {
...
func copyConfig(in *config.Akutan) *config.Akutan {
	bytes, err := json.Marshal(in)
	if err != nil {
		panic(fmt.Sprintf("Failed to marshal Akutan config for copy: %v", err))
	}
	out := new(config.Akutan)
	err = json.Unmarshal(bytes, out)
	if err != nil {
		panic(fmt.Sprintf("Failed to unmarshal Akutan config for copy: %v", err))
	}
	return out
}
