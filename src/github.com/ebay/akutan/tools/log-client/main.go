...
// Command log-client is a tool for low-level access to Akutan log servers. It's
...
const usage = `log-client is a command-line tool for low-level access to Akutan log servers.
...
Examples:
  ...
  # Akutan services will panic if the specified version is not supported.
...
func main() {
	...
	aLog, err := logspecclient.New(ctx, &config.Akutan{}, locator)
	if err != nil {
...
func info(ctx context.Context, aLog blog.AkutanLog, w io.Writer) error {
	fmt.Fprintf(w, "\nInvoking Info\n\n")
	start := clock.Now()
...
func appendVersion(ctx context.Context, aLog blog.AkutanLog, version int32, w io.Writer) error {
	fmt.Fprintf(w,
	...
	index, err := aLog.AppendSingle(ctx, data)
	if err != nil {
		return err
	}
	fmt.Fprintf(w,
...
func discardPrefix(ctx context.Context, aLog blog.AkutanLog, firstIndex uint64, w io.Writer) error {
	fmt.Fprintf(w, "\nInvoking Discard with firstIndex %v\n", firstIndex)
	start := clock.Now()
	err := aLog.Discard(ctx, firstIndex)
	if err != nil {
		return err
	}
