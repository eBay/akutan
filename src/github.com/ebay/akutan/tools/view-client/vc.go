...
import (
	...
	statsutil "github.com/ebay/akutan/util/stats"
...
const usage = `view-client is a command-line tool for calling Akutan views.
...
func main() {
	...
	if options.Index == 0 {
		log.Printf("Fetching last index from the log")
		aLog, err := blog.New(ctx, cfg)
		if err != nil {
			log.Fatalf("Error from blog.New: %v", err)
		}
