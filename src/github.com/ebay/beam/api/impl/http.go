...
import (
	...
	"github.com/ebay/beam/api/impl/kgstats"
...
// New returns a new instance of the API server, this exposes both an HTTP & gRPC API for consumers of the Beam system to use.
// The returned Server instance will not start handling traffic until a subsequent call to Server.Run()
func New(cfg *config.Beam, src *viewclient.Client, beamLog blog.BeamLog) *Server {
	s := &Server{
		cfg:     cfg,
		source:  src,
		beamLog: beamLog,
...
// Server is an implementation of the external gRPC & HTTP interfaces to Beam
type Server struct {
	cfg          *config.Beam
