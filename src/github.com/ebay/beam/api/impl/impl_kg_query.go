...
func (s *Server) QueryFacts(req *api.QueryFactsRequest, res api.BeamFactStore_QueryFactsServer) error {
...
// Query implements the query method in the BeamFactStore gRPC interface
func (s *Server) Query(req *api.QueryRequest, res api.BeamFactStore_QueryServer) error {
...
func (s *Server) query(ctx context.Context, req *api.QueryRequest, send func(*api.QueryResult) error) error {
	...
	index, err := s.resolveIndexConstraint(ctx, s.beamLog, req.Index)
	if err != nil {
		return err
	}
	ctx, cancel := context.WithCancel(ctx)
...
func toKGValue(o exec.Value) api.KGValue {
	...
	if o.KGObject.UnitID() != 0 {
		unit = &api.KGID{QName: o.UnitExtID, BeamId: o.KGObject.UnitID()}
	}
	...
	case rpc.KtKID:
		r.Value = &api.KGValue_Node{Node: &api.KGID{QName: o.ExtID, BeamId: o.KGObject.ValKID()}}
	case rpc.KtString:
		var lang *api.KGID
		if o.KGObject.LangID() != 0 {
			lang = &api.KGID{QName: o.LangExtID, BeamId: o.KGObject.LangID()}
		}
