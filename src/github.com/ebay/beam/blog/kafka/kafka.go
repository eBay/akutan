...
// Package kafka implements a Kafka client as a blog.BeamLog.
package kafka
...
// Log is a client to Kafka. It implements blog.BeamLog.
// TODO: many of these methods are misbehaved in that they handle errors
// differently than described in blog.BeamLog.
type Log struct {
...
func init() {
	blog.Factories["kafka"] = func(
		ctx context.Context, cfg *config.Beam, brokers discovery.Locator,
	) (blog.BeamLog, error) {
...
func NewLog(ctx context.Context, cfg *config.Beam, brokers discovery.Locator) (*Log, error) {
...
func (beamLog *Log) lastIndex(context.Context) (index int64, err error) {
	offset, err := beamLog.client.GetOffset(topic, 0, sarama.OffsetNewest)
	return offset, err
}

func (beamLog *Log) firstIndex(context.Context) (index int64, err error) {
	offset, err := beamLog.client.GetOffset(topic, 0, sarama.OffsetOldest)
	return offset + 1, err
}

// Info implements the method from blog.BeamLog.
func (beamLog *Log) Info(ctx context.Context) (*blog.Info, error) {
	var info blog.Info
	lastIndex, err := beamLog.lastIndex(ctx)
	if err != nil {
		return nil, err
	}
	info.LastIndex = uint64(lastIndex)
	firstIndex, err := beamLog.firstIndex(ctx)
	if err != nil {
		return nil, err
	}
...
// InfoStream implements the method from blog.BeamLog.
...
// Append implements the method from blog.BeamLog.
func (beamLog *Log) Append(ctx context.Context, msgs [][]byte) ([]blog.Index, error) {
	indexes := make([]blog.Index, len(msgs))
	for i := range msgs {
		_, offset, err := beamLog.producer.SendMessage(&sarama.ProducerMessage{
			Topic: topic,
...
// AppendSingle implements the method from blog.BeamLog.
func (beamLog *Log) AppendSingle(ctx context.Context, data []byte) (blog.Index, error) {
	indexes, err := beamLog.Append(ctx, [][]byte{data})
	if err != nil {
		return 0, err
	}
...
// Read implements the method from blog.BeamLog.
func (beamLog *Log) Read(ctx context.Context, startIndex blog.Index, entriesCh chan<- []blog.Entry) error {
	...
	for {
		fi, err1 := beamLog.firstIndex(ctx)
		li, err2 := beamLog.lastIndex(ctx)
		if err := errors.Any(err1, err2); err == nil {
			log.Infof("beamLog topic %s: firstIndex:%d, lastIndex:%d startConsumer startIndex:%d", topic, fi, li, startIndex)
			break
		} else {
	...
	consumer, err := beamLog.consumer.ConsumePartition(topic, 0, int64(startIndex)-1)
	if err != nil {
...
// Discard implements the method from blog.BeamLog.
func (beamLog *Log) Discard(ctx context.Context, startIndex blog.Index) error {
	...
	response, err := beamLog.broker.DeleteRecords(&request)
	if err != nil {
		return err
	}
