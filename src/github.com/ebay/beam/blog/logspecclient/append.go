...
// AppendSingle implements the method defined by blog.BeamLog.
...
// Append implements the method defined by blog.BeamLog.
func (beamLog *Log) Append(ctx context.Context, data [][]byte) ([]blog.Index, error) {
	...
	for i := range data {
		...
		case bytesLen(data[i]) > beamLog.appendBatchSize:
			addFuture(i)
			size = bytesLen(data[i])

		// [start:i] is full
		case size+bytesLen(data[i]) > beamLog.appendBatchSize:
			addFuture(i)
			size = bytesLen(data[i])
	...
	for _, future := range futures {
		...
		case beamLog.unbatchedAppends <- future:
		}
	}
...
func (beamLog *Log) batchAppends() {
	...
	for {
		...
		if len(overflow) == 0 {
			masked.unbatchedAppends = beamLog.unbatchedAppends
		}
		if len(batch) > 0 {
			masked.batchedAppends = beamLog.batchedAppends
		}
		...
		case future := <-masked.unbatchedAppends:
			...
			if batchBytes+futureBytes <= beamLog.appendBatchSize {
...
func (beamLog *Log) runAppends() {
	...
	for beamLog.ctx.Err() == nil {
		...
		if len(queue) > 0 && stream == nil {
			var err error
			stream, err = beamLog.newAppendStream()
			if err != nil { // ErrClosed or beamLog.ctx.Err()
				return
			}
		}
		...
		if stream == nil {
			logger = beamLog.logger.WithField("RPC", "Append")
		} else {
		...
		case res := <-masked.streamReplyCh:
			...
			case *logspec.AppendReply_Redirect:
				stream.cancel()
				_ = beamLog.handleRedirect(stream.client, res.Redirect)
				logger.WithFields(logrus.Fields{
			...
			case *logspec.AppendReply_Full:
				...
				beamLog.disconnectFrom(stream.client)
				_ = beamLog.clock.SleepUntil(beamLog.ctx,
					beamLog.clock.Now().Add(fullBackoff))
				reset()

			default: // unknown non-OK reply
				stream.cancel()
				logger.Warnf("Got unknown non-OK reply")
				beamLog.handleUnknownError(beamLog.ctx, stream.client)
				reset()
			}
		...
		case err := <-masked.streamErrCh:
			...
			beamLog.handleRPCError(beamLog.ctx, stream.client, err)
			reset()
		}
	}
}
...
type appendStream struct {
	logger *logrus.Entry
	// A child of beamLog.ctx. The stream.stream.Context() is a child of this one.
	ctx context.Context
	// Aborts 'ctx'.
	cancel  func()
	client  *clientConn
...
func (beamLog *Log) newAppendStream() (*appendStream, error) {
	for {
		logger := beamLog.logger.WithField("RPC", "Append")
		ctx, cancel := context.WithCancel(beamLog.ctx)
		...
		if err != nil {
			...
			beamLog.handleRPCError(ctx, client, err)
			continue
		}
		stream := &appendStream{
			logger:  logger,
		...
		go beamLog.sendAppendRequests(stream)
		go beamLog.recvAppendReplies(stream)
		return stream, nil
	}
}
...
func (beamLog *Log) sendAppendRequests(stream *appendStream) {
	defer stream.cancel()
	seq := uint64(0)
	for {
		seq++
...
func (beamLog *Log) recvAppendReplies(stream *appendStream) {
	defer stream.cancel()
	for {
