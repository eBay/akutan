...
func Test_Load(t *testing.T) {
	dir, err := ioutil.TempDir("", "beam-config-test")
	...
	t.Run("ok", func(t *testing.T) {
		err = ioutil.WriteFile(filepath.Join(dir, "ok.json"), []byte(`{
			"beamLog": {"type": "logspec"}
		}`), 0644)
		require.NoError(t, err)
		cfg, err := Load(filepath.Join(dir, "ok.json"))
		if assert.NoError(t, err) {
			assert.Equal(t, "logspec", cfg.BeamLog.Type)
		}
	})
}
...
func Test_Write(t *testing.T) {
	...
	// Happy path.
	err = Write(&Beam{}, filepath.Join(dir, "ok.json"))
	assert.NoError(t, err)

	// Simulate an error from encoder.Encode().
	marshalJSONErr = errors.New("ants in pants")
	err = Write(&Beam{Tracing: &Tracing{}}, filepath.Join(dir, "ants.json"))
	marshalJSONErr = nil
	if assert.Error(t, err) {
	...
	require.NoError(t, err)
	err = Write(&Beam{}, filepath.Join(dir, "subdir"))
	if assert.Error(t, err) {
