...
func New(cfg *config.Beam) (*DiskView, error) {
	if cfg.DiskView == nil {
		return nil, fmt.Errorf("diskView field missing in Beam config")
	}
	diskview := DiskView{
		cfg:     cfg,
	...
	if err != nil {
		log.Fatalf("Error reading beammeta key from db: %v", err)
	}
...
func (view *DiskView) Start() error {
	var err error
	view.beamLog, err = blog.New(context.TODO(), view.cfg)
	if err != nil {
		return err
	}
	...
	if view.cfg.DiskView.CarouselCatchupLag > 0 {
		...
		info, err := view.beamLog.Info(context.TODO())
		if err != nil {
...
func (view *DiskView) consumeLog() {
	...
	err := view.beamLog.Read(context.Background(), view.nextPosition.Index, ch)
	wait()
	if err != nil {
