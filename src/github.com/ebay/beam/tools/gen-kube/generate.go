...
// Command generate writes out Kubernetes configuration for portions of the Beam
...
var options struct {
	cfgFile        string
	beamImagesPath string
	beamImagesTag  string
	partitions     int
	replicas       int
}
...
func main() {
	...
	flag.StringVar(&options.cfgFile, "cfg", "cluster/k8s/config.json",
		"Beam config file to use as template")
	flag.StringVar(&options.beamImagesPath, "beam-images-path", "",
		"Prefix to the core Docker images. If empty, uses locally built images.")
	flag.StringVar(&options.beamImagesTag, "beam-images-tag", "latest",
	...
	cluster.Images.API = joinImagePath(options.beamImagesPath, "beam-api", options.beamImagesTag)
	cluster.Images.DiskView = joinImagePath(options.beamImagesPath, "beam-diskview", options.beamImagesTag)
	cluster.Images.TxView = joinImagePath(options.beamImagesPath, "beam-txview", options.beamImagesTag)
