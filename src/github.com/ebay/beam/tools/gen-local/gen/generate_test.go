...
func Test_Generate_minimal(t *testing.T) {
	...
	if assert.NoError(err) && assert.Len(cluster, 4) {
		assert.Equal("beam-api-00", cluster[0].Name)
		assert.Equal("beam-api", string(cluster[0].Type))
		assert.Equal(&config.Beam{
		...
		assert.Equal("beam-txview-00", cluster[1].Name)
		assert.Equal("beam-txview", string(cluster[1].Type))
		assert.Equal(&config.Beam{
			ViewsLocator: viewsLocator,
		...
		assert.Equal("beam-diskview-hashsp-00", cluster[2].Name)
		assert.Equal("beam-diskview", string(cluster[2].Type))
		assert.Equal(&config.Beam{
			ViewsLocator: viewsLocator,
		...
		assert.Equal("beam-diskview-hashpo-00", cluster[3].Name)
		assert.Equal("beam-diskview", string(cluster[3].Type))
		assert.Equal(&config.Beam{
			ViewsLocator: viewsLocator,
			DiskView: &config.DiskView{
