...
// Package logping measures the latency of Beam's log by appending to it and
...
type Options struct {
	...
	// Otherwise, it will open its own BeamLog with blog.New.
	...
	//
	// This feature is not available with every BeamLog instance (it requires an
	// optional Disconnect method). If the BeamLog instance doesn't support it or
...
// LogAppender defines a subset of the blog.BeamLog interface for appending.
// The Pinger uses a different BeamLog for reads.
type LogAppender interface {
	AppendSingle(ctx context.Context, data []byte) (blog.Index, error)
}

// logReader defines a subset of the blog.BeamLog interface for reading.
...
// the call to New. The cfg is used to construct BeamLog instances.
func New(
...
func (pinger *Pinger) Run() {
	for {
		...
		default:
			...
			// transient errors.
			log.WithError(err).Error("Pinger: Unexpected error reading Beam log. Restarting")
			metrics.restarts.Inc()
		}
	}
}
