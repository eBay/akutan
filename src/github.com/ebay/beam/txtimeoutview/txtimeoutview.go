...
import (
	...
	"github.com/ebay/beam/txtimeoutview/logping"
	"github.com/ebay/beam/txtimeoutview/txtimer"
	"github.com/ebay/beam/util/clocks"
	"github.com/ebay/beam/util/cmp"
	grpcserverutil "github.com/ebay/beam/util/grpc/server"
...
// the call to New.
func New(backgroundCtx context.Context, cfg *config.Beam) (*View, error) {
	if cfg.TxTimeoutView == nil {
		return nil, fmt.Errorf("txTimeoutView field missing in Beam config")
	}
	...
	v.beamLog, err = blog.New(ctx, cfg)
	if err != nil {
		abort()
		return nil, err
	}
	...
	v.txTimer = txtimer.New(ctx, v.beamLog, v.getReplayPosition, txtimer.Options{})
