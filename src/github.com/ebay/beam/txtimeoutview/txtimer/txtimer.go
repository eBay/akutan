...
// just for the call to New. The TxTimer will read from and append to beamLog.
func New(
...
// Run returns once the context given to New is canceled or the BeamLog given to
...
func (txTimer *TxTimer) Run() {
	for {
		...
		err = txTimer.beamLog.Read(txTimer.ctx, nextPosition.Index, entriesCh)
		wait()
		switch {
...
func (txTimer *TxTimer) abortTransactions() {
	...
	_, err := txTimer.beamLog.Append(txTimer.ctx, enc)
