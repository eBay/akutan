...
// A BeamLog that panics on all operations.
...
func (beamLog *readableLog) Read(ctx context.Context, nextIndex blog.Index, entriesCh chan<- []blog.Entry) error {
	return beamLog.read(ctx, nextIndex, entriesCh)
}
...
func (beamLog *appendableLog) Append(ctx context.Context, data [][]byte) ([]blog.Index, error) {
	return beamLog.append(ctx, data)
}
...
func Test_Run(t *testing.T) {
	...
	beamLog := new(readableLog)
	txTimer := New(ctx, beamLog, getReplayPosition, Options{})
	readCount := 0
	beamLog.read = func(ctx context.Context, nextIndex blog.Index, entriesCh chan<- []blog.Entry) error {
