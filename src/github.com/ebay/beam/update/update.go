...
import (
	...
	"github.com/ebay/beam/update/conv"
...
// BeamLog is the subset of blog.BeamLog that updates need. This makes it easier
// to write unit tests.
...
func Insert(ctx context.Context, req *api.InsertRequest,
	recentIdx blog.Index, store Store, beamLog BeamLog,
	...
	attempt := 1
	for {
		res, newReads, err := update.attempt(ctx, attempt, reads, store, beamLog)
		if err != nil {
			return nil, err
		}
		if res != nil {
...
func (update *updateRequest) attempt(ctx context.Context, attempt int,
	reads *updateReadSet, store Store, beamLog BeamLog,
	...
	txIdx, err := appendInsertTxBegin(ctx, beamLog, txCmd)
	if err != nil {
		return nil, nil, err
	}
	...
	default: // reads.Index < txIdx - 1
		...
		if !reads.equalValues(newReads) {
			...
			_, err = appendTxDecide(ctx, beamLog, txIdx, false)
			if err != nil {
				return nil, nil, err
			}
	...
	err = parallel.Invoke(ctx,
		func(ctx context.Context) error {
			_, err := appendTxDecide(ctx, beamLog, txIdx, true)
			return err
		},
		func(ctx context.Context) error {
...
func appendInsertTxBegin(ctx context.Context, beamLog BeamLog,
	...
	return beamLog.AppendSingle(ctx, enc)
}

// appendTxDecide appends a single tx decision message to the log and returns
// the index of the entry.
func appendTxDecide(ctx context.Context, beamLog BeamLog,
